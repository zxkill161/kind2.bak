...
(* 函数 parse_argv，用于解析命令行参数并进行相应的配置和处理。

函数的主要功能如下：

使用 CLAP（Command Line Argument Parsing）库解析命令行参数。
如果颜色选项为开启且不处于 XML 或 JSON 模式下，则设置输出的颜色。
执行命令行参数解析后的后续操作。
如果有任何模块信息的请求，打印模块信息并退出。
...
如果启用了证明或证明生成选项，则禁用压缩选项，并进行相应的日志记录。
根据选择的求解器执行特定的操作。
根据选择的求解器和量化消除求解器执行特定的操作。
如果启用了证明生成选项，则根据选定的求解器执行特定的操作。
如果启用了计算IVC选项并启用了压缩选项，则禁用压缩选项，并进行相应的日志记录。
如果启用了检查可达性选项，则根据其他选项执行相应的操作。
如果未启用检查可达性选项，则禁用检查非可真性选项。
 *)
